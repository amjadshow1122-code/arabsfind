...
import { Tag, Filter, SlidersHorizontal, ArrowRight, Lock } from 'lucide-react';
...
const CategoryBrowse = () => {
  ...
  const filteredSales = sales.filter(s => s.discount_max_pct >= discountFilter);
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <aside className="lg:w-60 shrink-0">
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-5">
              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-4">Browse Categories</p>
                ...
                {allCategories.map(cat => (
                  <Link
                    ...
                    className={`text-sm px-3 py-2 rounded-lg font-bold transition-all ${
                      cat.slug === slug ? 'bg-secondary/10 text-secondary' : 'text-gray-500 hover:text-primary hover:bg-gray-50'
                    }`}
                  >
                    {cat.name}
                  </Link>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              <div>
                <h1 className="text-3xl font-heading font-bold text-primary">
                  {category?.name || (slug ? slug.replace(/-/g, ' ') : 'All')}
                </h1>
            ...
            {loading ? (
              ...
              <>
                <p className="text-xs text-gray-400 font-bold mb-4 uppercase tracking-widest">
                  {filteredSales.length} active sale{filteredSales.length !== 1 ? 's' : ''}
                </p>
                {filteredSales.length === 0 ? (
                  <div className="bg-white rounded-xl border border-gray-100 p-16 text-center text-gray-400 italic text-sm">
                    No active sales in this category right now. Check back soon!
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
