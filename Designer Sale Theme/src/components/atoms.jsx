...
function Logo({ variant, size = 'md', showTagline = false, color }) {
    ...
    return (
      <a href="#/" className="logo-wrap" onClick={nav} style={{ display: 'inline-block' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: clr, gap: 6, lineHeight: 1 }}>
          ...
          {showTagline && (
            <span style={{
              fontFamily: 'var(--font-mono)', fontSize: tagFs, letterSpacing: '0.16em',
              textTransform: 'uppercase', color: 'var(--ink-soft)', marginTop: 10,
            }}>
              Australia's designer boutique sales
            </span>
          )}
        </div>
      </a>
  ...
  // ---- 'editorial' (default) ----
  // Clean Cormorant masthead. "Sale" italic + gold. ".com.au" tucked tight on
  // baseline as smaller italic afterthought — natural reading, no spacing.
  return (
    <a href="#/" className="logo-wrap" onClick={nav} style={{ display: 'inline-block' }}>
      <div style={{ display: 'flex', flexDirection: 'column', color: clr, lineHeight: 1 }}>
        <span style={{
          ...
          Designer<em style={{ fontStyle: 'italic', color: 'var(--gold-deep)' }}>Sale</em><em style={{
            fontStyle: 'italic',
            fontWeight: 400,
            fontSize: fs * 0.42,
            color: 'var(--gold-deep)',
...
// ---------- Image placeholder ----------
// A tonal panel + label. Hue varies per product for visual rhythm.
function FashionPlaceholder({ hue = 30, lightness = 80, label = 'editorial shot', shape = 'portrait' }) {
  ...
  return (
    <div className="ph-fashion pcard-img-inner" style={{
...
// Renders an <img> that fills its positioned parent. If the image errors,
// falls back to the tonal placeholder so the layout never breaks.
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      <div style={{
        ...
        inset: 0,
        background:
          'linear-gradient(135deg, #E8D4B8 0%, #C9A87C 35%, #8B6B45 70%, #5C4632 100%)',
      }} />
      {/* tonal stripes */}
      <div style={{
        position: 'absolute',
      ...
      <div style={{
        ...
        background: 'radial-gradient(120% 80% at 70% 30%, rgba(255,255,255,0.18) 0%, rgba(0,0,0,0.0) 50%, rgba(0,0,0,0.35) 100%)',
      }} />
      <div style={{
